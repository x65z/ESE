	...
	while (Sin != 'q')
	{
		...
		switch(Sin)	
		{
			case '1' :	//send text
			{
				...
				printf("\nOutputting data\n");
				initPort();										// Initialize the port
				outputToPort(msgOut, strlen(msgOut)); //Outputting
				purgePort();									// Purge the port
				CloseHandle(hCom);
				break;
			}
				...
				if(count > 0)
				{
					if(rptr->bBools[0] == 0)
					{
						...
						if(count > 0)
						{
							...
							if(rptr->bReceiverAddr == rptr->bReceiverAddrb)
							{
								printf("No errors in receiver address.\n");
							}
							else
							{
								printf("\nErrors detected in receiver address, a is %d, b is %d.\n",rptr->bReceiverAddr, rptr->bReceiverAddrb);
							}
							if(rptr->bSenderAddr == rptr->bSenderAddrb)
							{
							...
							{
								printf("\nErrors detected in sender address, a is %d, b is %d.\n",rptr->bSenderAddr, rptr->bSenderAddrb);
							}
							create_node(msgIn);
							char mFlag = '0';
							iText++;
