...
void create_phone_node(BYTE bSenderAddr) //phonebook
{
	//printf("\nDebug: A\n");
	...
	{
		find_ordered_tailp(bSenderAddr);
		if (bSenderAddr < tailp->bSenderAddr)
		{
			tailp->nextL = ptr;
		} 
		else
...
//Sets tail phonebook
void find_ordered_tailp(BYTE bSenderAddr)
{
	tail = head;
	while (tail != NULL) //Searches alphabetically until finds a NULL
	{
...
void display_listp(char flag) //Uses recursion to display phone book
{
	...
	printf("\n");
	printf("%d", dptr->bSenderAddr);	// Printing Messages 
