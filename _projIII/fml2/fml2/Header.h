...
char bReceiverAddr[5]; // receiver ID. 0xff=broadcast
char bReceiverAddrb[5]; // receiver ID. 0xff=broadcast
char bSenderAddr[5]; // receiver ID. 0xff=broadcast
char bSenderAddrb[5]; // receiver ID. 0xff=broadcast
int hash; //For checking text
int priority;//from 1-7
...
void find_ordered_tailp(char bSenderAddr); //phonebook
void search(char fbuf);
int height_list(char flag);
void display_list();
void displayprior_list(); //Displayh with priority
void display_listp(char flag);//phonebook
